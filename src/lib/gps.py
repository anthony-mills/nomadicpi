...
def get_gps():
    ...
    if gpsd_socket is None:
        try:
            gps_connect(host=gps_host, port=gps_port)
        except: 
            logger.info("Unable to connect to GPSD service at: " + str(gps_host) + ":" + str(gps_port))
    
    gps_info = get_current()
    
    try:
        cur_pos = gps_info.position()
        
        if len(cur_pos) == 2:
             cur_gps['latitude'] = cur_pos[0]
             cur_gps['longitude'] = cur_pos[1]
                
        cur_gps['speed'] = gps_info.speed()
        cur_gps['utc'] = str(gps_info.get_time())
        cur_gps['time'] = str(gps_info.get_time(local_time=True))
        cur_gps['altitude'] = gps_info.altitude()  
        cur_gps['precision'] = gps_info.position_precision()
        cur_gps['direction'] = gps_info.movement()
    except NoFixError:
        logger.info("Need a gps fix for altitude")        
                      
    try:
        cur_gps['altitude'] = gps_info.altitude()
    except NoFixError:
        logger.info("Need a 3d fix for altitude")

    threading.Timer(1, get_gps).start()
